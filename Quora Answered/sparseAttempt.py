...
def form_feature_vec(jso, topicset, topiclist, sample, i):
    """ Form feature vector from json (not including __ans__) """
    ...
    if jso['context_topic'] :#and jso['context_topic']['name'] in topicset:
        sample[i, 4] = topiclist.index(jso['context_topic']['name'])
        sample[i, 5] = jso['context_topic']['followers']
    else:
        sample[i, 4:6] = [-1, 0]
...
samples = sparse.dok_matrix((N, 6), dtype=int)
...
sample = sparse.dok_matrix((1,6), dtype=int)
for jso in test_json:
    form_feature_vec(jso, topicset, topiclist, sample, 0)
...
# Sparse attempt.  No good because the RandomForestClassifier requires dense data